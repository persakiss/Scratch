...
const numbersAddFive = numbers.map(el => el += 5)
...
const numbersReformatted = numbers.map(el => `${el}: `)
...
// Bonus:
const capitalizedWords = words.map(word => (word.slice(0,1).toUpperCase() + word.slice(1,word.length)))
